...
def test_integration_buddy_tool_loader(script_dir, caplog, default_config_loader):
    ...
    assert "Executing the following modules in parallel: ['Aspell', 'Chktex', " \
           "'Diction', 'LanguageTool', 'LogFilter', 'CheckFigureResolution', " \
           "'EmptySections', 'NativeUseOfRef', 'SiUnitx', 'URLCheck', " \
           "'UnreferencedFigures', 'ProseLint', 'YaLafi']" in records
